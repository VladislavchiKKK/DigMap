...
// Використовуємо CartoDB Positron для чистої, світлої карти
        ...
        if (!res.ok) {
            ...
            if (Array.isArray(data)) {
                errorMessage = data.map(err => {
                    ...
                    if (err.code === "PasswordRequiresDigit") return "⚠️ Пароль має містити хоча б одну цифру.";
                    if (err.code === "PasswordRequiresUpper") return "⚠️ Пароль має містити велику літеру.";
                    return "⚠️ " + err.description;
                }).join("\n");
            }
...
async function loadFinds() {
    try {
        ...
        finds.forEach(item => {
            ...
            let badgeClass = item.$type === "coin" ? "badge-coin" : "badge-artifact";
            let badgeText = item.$type === "coin" ? "💰 Монета" : "🏺 Артефакт";
            let borderClass = item.$type === "coin" ? "border-coin" : "border-artifact";
